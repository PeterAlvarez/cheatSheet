...
//  }
	...
	//devuelve todos los  nodos conectados directa o indirectamente a el 
...
//              System.out.println(nodo);
	...
	//RETORNAR FALSO SI EL GRAFO NO ES CONEXO
	public boolean conexo() {
		boolean sal = true;
		for (int i = 0; i < getNodosTotal(); i++) {
			if (amplitud(nodos[i]).size() != getNodosTotal()) {
				sal = false;
				break;
			}
		}
	...
	public void eliminarNodosTerminales() {
		for( int i=0 ; i<n ; i++ ) {		
			if(nodosAdyascentes(nodos[i]).size()==1  ) {
				eliminarNodo(nodos[i]);
			}
		}
	}
	
	public String nodosDeGradoN(int dato) {
		String out = "";
		for( int i=0 ; i<n ; i++ ) {		
			if(nodosAdyascentes(nodos[i]).size() == dato  ) {
				out += nodos[i]+" ";
			}
		}
		return out;
	...
	//desde aqui es examen recuperatorio
	public int gradoGrafo() {
		int maximoAristas=0;
		for( int i=0 ; i<n ;i++ ) {
			if (maximoAristas<nodosAdyascentes(nodos[i]).size()) {
				maximoAristas=nodosAdyascentes(nodos[i]).size();
			}
		}
		return maximoAristas;
	}
	
	public boolean tamSubGrafo() {
		for( int i=0 ; i<n ; i++) {
			if (amplitud(nodos[i]).size()==3) {
				return true;
			}
		}
		return false;
	...
	public boolean subgrafoGrado3() {
		if ( conexo()==false ||  gradoGrafo()==3 || tamSubGrafo()) {
			return true;
		}else {
			return false;	
		}
	...
	public void eliminarNodoMinimo(){
		...
		String aux=nodos[0];
		for( int i=0 ; i<n ; i++ ) {
			if(menor>nodosAdyascentes(nodos[i]).size()) {
				menor=nodosAdyascentes(nodos[i]).size();
				aux=nodos[i];
			}
		}
		eliminarNodo(aux);

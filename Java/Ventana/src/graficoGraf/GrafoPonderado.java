...
public class GrafoPonderado {
    ...
    public void matrizDeRecorridoOP(){
        for( int i=0 ; i<n ; i++ ) {
            for( int j=0 ;j<n ; j++ ) {
                mat2[i][j]=nodos[j];
            }
            ...
            for( int i=0 ; i<n ;i++ ) {
                for( int j=0 ; j<n ;j++ ) {
                    if( mat[i][k]+mat[k][j]<mat[i][j] ) {
                        ...
                        mat2[j][i]=nodos[k];// llena datos en matriz de Recorrido
                    }
                }
            }
        }
    ...
    //    retorna Distancia corta
    public int encuentraDistancia( String arist1,String arist2  ) {

        int aux=indiceNodo(arist1);
        int aux2=indiceNodo(arist2);
        return (int) mat[aux][aux2];
        //    	System.out.println(mat[aux][aux2]);
    }

    public String encuentraCamino( String arist1,String arist2 ) {

        int filmayor = indiceNodo(arist1);
        int colmenor = indiceNodo(arist2);

        if( filmayor<colmenor ) {
            filmayor=indiceNodo(arist2);
            colmenor=indiceNodo(arist1);
        }

        String cad = nodos[filmayor];
        int ind = colmenor;
        for(int i=indiceNodo(nodos[filmayor]) ; i>=0 ; i--) {
            if( mat2[i][colmenor]==nodos[colmenor] ) {
                cad+=" => "+mat2[i][colmenor];
                break;
            } else if ( mat2[ i-1 ][colmenor]!=mat2[i][colmenor] )
                cad += " => "+mat2[i][colmenor];
        }
        return cad;
